...
interface Parent {
  ...
  OrgtnalSruItem?: Child[]; // An array of OrgtnalSruItem objects
...
const DeleteOriStru: React.FC<Props> = () => {
  ...
  const deleteItem = async (itemId: string) => {
    try {
      const response = await fetch(
        `http://165.22.219.69:5002/api/delete/prioritysport/${itemId}/${profileId}/${Key_Key}`,
        {
          method: "PATCH",
          credentials: "include",
        }
  ...
  return (
    <>
      <>
        <Card>
          <Card.Body>
            <h4>Delete organizational Structure Page</h4>
...
const UpdatePrioSpotItem: React.FC<Props> = ({
  itemId,
  parentId,
  innerdata,
}) => {
  ...
  const UpdateProtySpotItem = async (
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/update/prioritysport/${parentId}/prioritysportitem/${itemId}/${profileId}/${Key_Key}`,
          {
            method: "PATCH",
            body: formData,
            credentials: "include",
...
const DeletePrioSpotPage: React.FC<Parent> = ({ id }) => {
  const [validated, setValidated] = useState<boolean>(false);
  ...
  const deleteItem = async (itemId: string) => {
    try {
      const response = await fetch(
        `http://165.22.219.69:5002/api/delete/prioritysport/${id}/prioritysportitem/${itemId}/${profileId}/${Key_Key}`,
        {
          method: "PATCH",
          credentials: "include",
        }
  ...
  useEffect(() => {
    fetch(`http://165.22.219.69:5002/api/prioritysportitem/${id}`)
      .then((response) => response.json())
  ...
  return (
    <>
      {isEditItem ? (
        ...
        <>
          <h4>Update or deletePriority Sport data</h4>
          ...
          <Card>
            <Card.Body>
              <Table striped bordered hover>
                ...
                {(innerdata || [])
                  .sort((a, b) => {
                    // Ensure that 'created_at' is a valid date and convert it to a Date object
...
const AddPrioSpotPageItem: React.FC<Parent> = ({ id }) => {
  const [validated, setValidated] = useState<boolean>(false);
  const [sportName, setSportName] = useState("");
  const [sportDetails, setSportDetails] = useState("");
  const [image, setImage] = useState<File | null>(null);
  ...
  const addOrgItem = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/prioritysportitem/${id}/${profileId}/${Key_Key}`,
          {
            method: "POST",
            body: formData,
            credentials: "include",
        ...
        <Card.Body>
          ...
          >
            ...
            <Form.Group>
              <Form.Label className="d-flex  pt-2justify-content-start font-weight-bold">
                <h5>
                  Sport Details <span style={{ color: "red" }}> *</span>
                </h5>
              </Form.Label>
              <Form.Control
                className="accordion-item"
...
const PrioritySport = () => {
  ...
  const handleFormSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/prioritysport/${profileId}/${Key_Key}`,
          {
            method: "POST",
            body: formData,
            credentials: "include",
  ...
  const handleUpdate = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/update/prioritysport/${objectId}/${profileId}/${Key_Key}`,
          {
            method: "PATCH",
            body: formData,
            credentials: "include",
      ...
      <div>
        <Card>
          <Card.Body>
            ...
            <Row>
              <Col lg={6}>
                <AddPrioSpotPageItem id={objectId} />
              </Col>
              <Col lg={6}>
                {objectId && data && <DeletePrioSpotPage id={objectId} />}
              </Col>
            </Row>
          </Card.Body>
        </Card>
