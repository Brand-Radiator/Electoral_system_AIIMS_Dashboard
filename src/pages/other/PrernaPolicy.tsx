...
const DeletePrenaPage: React.FC<Parent> = ({ userData }) => {
  ...
  const deleteItem = async (itemId: string) => {
    try {
      const response = await fetch(
        `http://165.22.219.69:5002/api/delete/prena/${itemId}/${profileId}/${Key_Key}`,
        {
          method: "PATCH",
          credentials: "include",
        }
  ...
  return (
    <>
      <>
        <Card>
          <Card.Body>
            <h4>Delete prena Policy Page</h4>
...
const UpdatePrenaItem: React.FC<Props> = ({ itemId, parentId, innerdata }) => {
  const [validated, setValidated] = useState<boolean>(false);
  ...
  const UpdateProtySpotItem = async (
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/update/prena/${parentId}/prenaitem/${itemId}/${profileId}/${Key_Key}`,
          {
            method: "PATCH",
            body: formData,
            credentials: "include",
...
const DeletePrenaPageItem: React.FC<Parent> = ({ id }) => {
  const [validated, setValidated] = useState<boolean>(false);
  ...
  useEffect(() => {
    fetch(`http://165.22.219.69:5002/api/prena/${id}`)
      .then((response) => response.json())
  ...
  const deleteItem = async (itemId: string) => {
    try {
      const response = await fetch(
        `http://165.22.219.69:5002/api/delete/prena/${id}/prenaitem/${itemId}/${profileId}/${Key_Key}`,
        {
          method: "PATCH",
          credentials: "include",
        }
  ...
  return (
    <>
      {isEditItem ? (
        ...
        <>
          <h4>Update or delete prena Data</h4>
...
const AddPrenaPageItem: React.FC<Parent> = ({ id }) => {
  const [validated, setValidated] = useState<boolean>(false);
  const [sportName, setSportName] = useState("");
  const [sportDetails, setSportDetails] = useState("");
  const [image, setImage] = useState<File | null>(null);
  ...
  const addOrgItem = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/prena/${id}/${profileId}/${Key_Key}`,
          {
            method: "POST",
            body: formData,
            credentials: "include",
...
const Prena = () => {
  ...
  useEffect(() => {
    // fetch("http://165.22.219.69:5002/api/Prena")
    fetch("http://165.22.219.69:5002/api/prena")
      .then((response) => response.json())
      .then((res) => setData(res));
  }, []);
  ...
  const handleFormSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/prena/${profileId}/${Key_Key}`,
          {
            method: "POST",
            body: formData,
            credentials: "include",
  ...
  const handleUpdate = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/update/prena/${objectId}/${profileId}/${Key_Key}`,
          {
            method: "PATCH",
            body: formData,
            credentials: "include",
        ...
        <Card>
          <Card.Body>
            <Row>
              {data.length === 0 ? (
                ...
                <Col lg={6}>
                  <Form
                    ...
                    encType="multipart/form-data"
                  >
                    <h4>Update prena Policy Page</h4>
                    {isError && (
                      <Alert variant="danger" className="my-2">
                        {isResponse}
                      </Alert>
              ...
              {/* -------------------------- update data--------------------------------------------- */}
              <Col lg={6}>{data && <DeletePrenaPage userData={data} />}</Col>
            </Row>
            <div
            ...
            <Row>
              <Col lg={6}>
                <AddPrenaPageItem id={objectId} />
              </Col>
              <Col lg={6}>
                {objectId && data && <DeletePrenaPageItem id={objectId} />}
              </Col>
            </Row>
          </Card.Body>
        </Card>
