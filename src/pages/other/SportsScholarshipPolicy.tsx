...
const DeleteSportScholarshipPolicyPage: React.FC<Parent> = ({ userData }) => {
  ...
  const deleteItem = async (itemId: string) => {
    try {
      const response = await fetch(
        `http://165.22.219.69:5002/api/delete/sportscholarshippolcy/${itemId}/${profileId}/${Key_Key}`,
        {
          method: "PATCH",
          credentials: "include",
        }
  ...
  return (
    <>
      <>
        <Card>
          <Card.Body>
            <h4>Delete Sport ScholarShip Policy Page</h4>
...
const UpdateSportScholarshipPolicyItem: React.FC<Props> = ({
  itemId,
  parentId,
  innerdata,
}) => {
  ...
  const UpdateProtySpotItem = async (
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/update/sportscholarshippolcy/${parentId}/sportscholarshippolcyitem/${itemId}/${profileId}/${Key_Key}`,
          {
            method: "PATCH",
            body: formData,
            credentials: "include",
...
const DeleteSportScholarshipPolicyPageItem: React.FC<Parent> = ({ id }) => {
  const [validated, setValidated] = useState<boolean>(false);
  ...
  useEffect(() => {
    fetch(`http://165.22.219.69:5002/api/sportscholarshippolcy/${id}`)
      .then((response) => response.json())
  ...
  const deleteItem = async (itemId: string) => {
    try {
      const response = await fetch(
        `http://165.22.219.69:5002/api/delete/sportscholarshippolcy/${id}/sportscholarshippolcyitem/${itemId}/${profileId}/${Key_Key}`,
        {
          method: "PATCH",
          credentials: "include",
        }
    ...
    <>
      {isEditItem ? (
        <Row>
          <Col lg={10}>
            {innerdata && (
              <UpdateSportScholarshipPolicyItem
                itemId={isEditItem}
                parentId={id}
                innerdata={innerdata}
              />
        ...
        <>
          <h4>Update or delete Sport ScholarShip Data</h4>
...
const AddSportScholarshipPolicyPageItem: React.FC<Parent> = ({ id }) => {
  const [validated, setValidated] = useState<boolean>(false);
  const [sportName, setSportName] = useState("");
  const [sportDetails, setSportDetails] = useState("");
  const [image, setImage] = useState<File | null>(null);
  ...
  const addOrgItem = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/sportscholarshippolcy/${id}/${profileId}/${Key_Key}`,
          {
            method: "POST",
            body: formData,
            credentials: "include",
      ...
      <Card>
        <Card.Body>
          <Form
            ...
            encType="multipart/form-data"
          >
            <h4>Add Sport ScholarShip Item</h4>
            {isError && (
              <Alert variant="danger" className="my-2">
                {isResponse}
              </Alert>
            ...
            <Form.Group>
              <Form.Label className="d-flex pt-1 justify-content-start">
                <h5>
                  Image/File <span style={{ color: "red" }}> *</span>
                </h5>
              </Form.Label>
              <Form.Control
                type="file"
...
const SportScholarshipPolicy = () => {
  ...
  useEffect(() => {
    // fetch("http://165.22.219.69:5002/api/SportScholarshipPolicy")
    fetch("http://165.22.219.69:5002/api/sportscholarshippolcy")
      .then((response) => response.json())
      .then((res) => setData(res));
  }, []);
  ...
  const handleFormSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/sportscholarshippolcy/${profileId}/${Key_Key}`,
          {
            method: "POST",
            body: formData,
            credentials: "include",
  ...
  const handleUpdate = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/update/sportscholarshippolcy/${objectId}/${profileId}/${Key_Key}`,
          {
            method: "PATCH",
            body: formData,
            credentials: "include",
        ...
        <Card>
          <Card.Body>
            <Row>
              {data.length === 0 ? (
                <Col lg={6}>
                  <Form
                    ...
                    encType="multipart/form-data"
                  >
                    <h4>Add Sport ScholarShip Policy Page</h4>
                    {isError && (
                      <Alert variant="danger" className="my-2">
                        {isResponse}
                      </Alert>
                ...
                <Col lg={6}>
                  <Form
                    ...
                    encType="multipart/form-data"
                  >
                    <h4>Update Sport ScholarShip Policy Page</h4>
                    {isError && (
                      <Alert variant="danger" className="my-2">
                        {isResponse}
                      </Alert>
              ...
              {/* -------------------------- update data--------------------------------------------- */}
              <Col lg={6}>
                {data && <DeleteSportScholarshipPolicyPage userData={data} />}
              </Col>
            </Row>
            <div
            ...
            <Row>
              <Col lg={6}>
                <AddSportScholarshipPolicyPageItem id={objectId} />
              </Col>
              <Col lg={6}>
                {objectId && data && (
                  <DeleteSportScholarshipPolicyPageItem id={objectId} />
                )}
              </Col>
            </Row>
          </Card.Body>
...
export default SportScholarshipPolicy;
