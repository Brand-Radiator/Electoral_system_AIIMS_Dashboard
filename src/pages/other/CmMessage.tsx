  ...
  const deleteItem = async (itemId: string) => {
    try {
      const response = await fetch(
        `http://165.22.219.69:5002/api/delete/cmmessage/${itemId}/${profileId}/${Key_Key}`,
        {
          method: "PATCH",
          credentials: "include",
        }
...
const Introduction = () => {
  ...
  const handleFormSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/cmmessage/${profileId}/${Key_Key}`,
          {
            method: "POST",
            body: formData,
            credentials: "include",
  ...
  const handleUpdate = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/update/cmmessage/${objectId}/${profileId}/${Key_Key}`,
          {
            method: "PATCH",
            body: formData,
            credentials: "include",
