...
const DeleteNationalAchievementPage: React.FC<Parent> = ({ userData }) => {
  ...
  const deleteItem = async (itemId: string) => {
    try {
      const response = await fetch(
        `http://165.22.219.69:5002/api/delete/Nationalachievement/${itemId}/${profileId}/${Key_Key}`,
        {
          method: "PATCH",
          credentials: "include",
        }
  ...
  return (
    <>
      <>
        <Card>
          <Card.Body>
            <h4>Delete National Level Achievement Page</h4>
...
const UpdateNationalAchievementItem: React.FC<Props> = ({
  itemId,
  parentId,
  innerdata,
}) => {
  ...
  const UpdateProtySpotItem = async (
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/update/nationalachievement/${parentId}/nationalachievementitem/${itemId}/${profileId}/${Key_Key}`,
          {
            method: "PATCH",
            body: formData,
            credentials: "include",
...
const DeleteNationalAchievementPageItem: React.FC<Parent> = ({ id }) => {
  const [validated, setValidated] = useState<boolean>(false);
  ...
  useEffect(() => {
    fetch(`http://165.22.219.69:5002/api/nationalachievement/${id}`)
      .then((response) => response.json())
  ...
  const deleteItem = async (itemId: string) => {
    try {
      const response = await fetch(
        `http://165.22.219.69:5002/api/delete/nationalachievement/${id}/nationalachievementitem/${itemId}/${profileId}/${Key_Key}`,
        {
          method: "PATCH",
          credentials: "include",
        }
  ...
  return (
    <>
      {isEditItem ? (
        ...
        <>
          <h4>Update or delete National Level Achievement Data</h4>
...
const AddNationalAchievementPageItem: React.FC<Parent> = ({ id }) => {
  const [validated, setValidated] = useState<boolean>(false);
  const [sportName, setSportName] = useState("");
  const [sportDetails, setSportDetails] = useState("");
  const [image, setImage] = useState<File | null>(null);
  ...
  const addOrgItem = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/nationalachievement/${id}/${profileId}/${Key_Key}`,
          {
            method: "POST",
            body: formData,
            credentials: "include",
      ...
      <Card>
        <Card.Body>
          <Form
            ...
            encType="multipart/form-data"
          >
            <h4>Add National Level Achievement Item</h4>
            {isError && (
              <Alert variant="danger" className="my-2">
                {isResponse}
              </Alert>
...
const NationalAchievement = () => {
  ...
  useEffect(() => {
    fetch("http://165.22.219.69:5002/api/nationalachievement")
      .then((response) => response.json())
      .then((res) => setData(res));
  }, []);
  ...
  const handleFormSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/nationalachievement/${profileId}/${Key_Key}`,
          {
            method: "POST",
            body: formData,
            credentials: "include",
  ...
  const handleUpdate = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/update/nationalachievement/${objectId}/${profileId}/${Key_Key}`,
          {
            method: "PATCH",
            body: formData,
            credentials: "include",
        ...
        <Card>
          <Card.Body>
            <Row>
              {data.length === 0 ? (
                ...
                <Col lg={6}>
                  <Form
                    ...
                    encType="multipart/form-data"
                  >
                    <h4>Update National Level Achievement Page</h4>
                    {isError && (
                      <Alert variant="danger" className="my-2">
                        {isResponse}
                      </Alert>
              ...
              {/* -------------------------- update data--------------------------------------------- */}
              <Col lg={6}>
                {data && <DeleteNationalAchievementPage userData={data} />}
              </Col>
            </Row>
            <div
            ...
            <Row>
              <Col lg={6}>
                <AddNationalAchievementPageItem id={objectId} />
              </Col>
              <Col lg={6}>
                {objectId && data && (
                  <DeleteNationalAchievementPageItem id={objectId} />
                )}
              </Col>
            </Row>
          </Card.Body>
