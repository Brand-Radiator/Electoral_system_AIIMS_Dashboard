...
const DeleteudaanPage: React.FC<Parent> = ({ userData }) => {
  ...
  const deleteItem = async (itemId: string) => {
    try {
      const response = await fetch(
        `http://165.22.219.69:5002/api/delete/udaan/${itemId}/${profileId}/${Key_Key}`,
        {
          method: "PATCH",
          credentials: "include",
        }
  ...
  return (
    <>
      <>
        <Card>
          <Card.Body>
            <h4>Delete udaan Policy Page</h4>
...
const UpdateudaanItem: React.FC<Props> = ({ itemId, parentId, innerdata }) => {
  const [validated, setValidated] = useState<boolean>(false);
  ...
  const UpdateProtySpotItem = async (
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/update/udaan/${parentId}/udaanitem/${itemId}/${profileId}/${Key_Key}`,
          {
            method: "PATCH",
            body: formData,
            credentials: "include",
...
const DeleteudaanPageItem: React.FC<Parent> = ({ id }) => {
  const [validated, setValidated] = useState<boolean>(false);
  ...
  useEffect(() => {
    fetch(`http://165.22.219.69:5002/api/udaan/${id}`)
      .then((response) => response.json())
  ...
  const deleteItem = async (itemId: string) => {
    try {
      const response = await fetch(
        `http://165.22.219.69:5002/api/delete/udaan/${id}/udaanitem/${itemId}/${profileId}/${Key_Key}`,
        {
          method: "PATCH",
          credentials: "include",
        }
  ...
  return (
    <>
      {isEditItem ? (
        ...
        <>
          <h4>Update or delete udaan Data</h4>
...
const AddudaanPageItem: React.FC<Parent> = ({ id }) => {
  const [validated, setValidated] = useState<boolean>(false);
  const [sportName, setSportName] = useState("");
  const [sportDetails, setSportDetails] = useState("");
  const [image, setImage] = useState<File | null>(null);
  ...
  const addOrgItem = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/udaan/${id}/${profileId}/${Key_Key}`,
          {
            method: "POST",
            body: formData,
            credentials: "include",
...
const Udaan = () => {
  ...
  useEffect(() => {
    // fetch("http://165.22.219.69:5002/api/udaan")
    fetch("http://165.22.219.69:5002/api/udaan")
      .then((response) => response.json())
      .then((res) => setData(res));
  }, []);
  ...
  const handleFormSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/udaan/${profileId}/${Key_Key}`,
          {
            method: "POST",
            body: formData,
            credentials: "include",
  ...
  const handleUpdate = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/update/udaan/${objectId}/${profileId}/${Key_Key}`,
          {
            method: "PATCH",
            body: formData,
            credentials: "include",
        ...
        <Card>
          <Card.Body>
            <Row>
              {data.length === 0 ? (
                ...
                <Col lg={6}>
                  <Form
                    ...
                    encType="multipart/form-data"
                  >
                    <h4>Update udaan Policy Page</h4>
                    {isError && (
                      <Alert variant="danger" className="my-2">
                        {isResponse}
                      </Alert>
              ...
              {/* -------------------------- update data--------------------------------------------- */}
              <Col lg={6}>{data && <DeleteudaanPage userData={data} />}</Col>
            </Row>
            <div
            ...
            <Row>
              <Col lg={6}>
                <AddudaanPageItem id={objectId} />
              </Col>
              <Col lg={6}>
                {objectId && data && <DeleteudaanPageItem id={objectId} />}
              </Col>
            </Row>
          </Card.Body>
        </Card>
