...
const DeleteUpcomingEventPage: React.FC<Parent> = ({ userData }) => {
  ...
  const deleteItem = async (itemId: string) => {
    try {
      const response = await fetch(
        `http://165.22.219.69:5002/api/delete/UpcomingEvent/${itemId}/${profileId}/${Key_Key}`,
        {
          method: "PATCH",
          credentials: "include",
        }
  ...
  return (
    <>
      <>
        <Card>
          <Card.Body>
            <h4>Delete Upcoming Event Page</h4>
...
const UpdateUpcomingEventItem: React.FC<Props> = ({
  itemId,
  parentId,
  innerdata,
}) => {
  ...
  const UpdateProtySpotItem = async (
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/update/Upcomingevent/${parentId}/upcomingeventitem/${itemId}/${profileId}/${Key_Key}`,
          {
            method: "PATCH",
            body: formData,
            credentials: "include",
...
const DeleteUpcomingEventPageItem: React.FC<Parent> = ({ id }) => {
  const [validated, setValidated] = useState<boolean>(false);
  ...
  useEffect(() => {
    fetch(`http://165.22.219.69:5002/api/upcomingevent/${id}`)
      .then((response) => response.json())
  ...
  const deleteItem = async (itemId: string) => {
    try {
      const response = await fetch(
        `http://165.22.219.69:5002/api/delete/upcomingevent/${id}/upcomingeventitem/${itemId}/${profileId}/${Key_Key}`,
        {
          method: "PATCH",
          credentials: "include",
        }
...
const AddUpcomingEventPageItem: React.FC<Parent> = ({ id }) => {
  const [validated, setValidated] = useState<boolean>(false);
  const [sportName, setSportName] = useState("");
  ...
  const addOrgItem = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        // const response = await fetch(`http://165.22.219.69:5002/api/upcomingevent/${id}`, {
        const response = await fetch(
          `http://165.22.219.69:5002/api/upcomingevent/${id}/${profileId}/${Key_Key}`,
          {
            method: "POST",
            body: formData,
            credentials: "include",
...
const UpcomingEvent = () => {
  ...
  useEffect(() => {
    // fetch("http://165.22.219.69:5002/api/UpcomingEvent")
    fetch("http://165.22.219.69:5002/api/UpcomingEvent")
      .then((response) => response.json())
      .then((res) => setData(res));
  }, []);
  ...
  const handleFormSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        // const response = await fetch(`http://165.22.219.69:5002/api/UpcomingEvent`, {
        const response = await fetch(
          `http://165.22.219.69:5002/api/UpcomingEvent/${profileId}/${Key_Key}`,
          {
            method: "POST",
            body: formData,
            credentials: "include",
  ...
  const handleUpdate = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          // `http://165.22.219.69:5002/api/update/UpcomingEvent/${objectId}`,
          `http://165.22.219.69:5002/api/update/UpcomingEvent/${objectId}/${profileId}/${Key_Key}`,
        ...
        <Card>
          <Card.Body>
            <Row>
              {data.length === 0 ? (
                ...
                <Col lg={6}>
                  <Form
                    ...
                    encType="multipart/form-data"
                  >
                    <h4>Update Upcoming Event Page</h4>
                    {isError && (
                      <Alert variant="danger" className="my-2">
                        {isResponse}
                      </Alert>
              ...
              {/* -------------------------- update data--------------------------------------------- */}
              <Col lg={6}>
                {data && <DeleteUpcomingEventPage userData={data} />}
              </Col>
            </Row>
            <div
            ...
            <Row>
              <Col lg={6}>
                <AddUpcomingEventPageItem id={objectId} />
              </Col>
              <Col lg={6}>
                {objectId && data && (
                  <DeleteUpcomingEventPageItem id={objectId} />
                )}
              </Col>
            </Row>
          </Card.Body>
