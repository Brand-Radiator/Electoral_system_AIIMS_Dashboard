...
const DeleteOriStru = () => {
  ...
  const deleteItem = async (itemId: string) => {
    try {
      const response = await fetch(
        `http://165.22.219.69:5002/api/delete/orgstructure/${itemId}/${profileId}/${Key_Key}`,
        {
          method: "PATCH",
          credentials: "include",
        }
  ...
  useEffect(() => {
    fetch("http://165.22.219.69:5002/api/orgstructure")
      // fetch("http://165.22.219.69:5002/api/orgstructure")
...
const UpdateOriStruItem: React.FC<UpdateServiceSectionProps> = ({
  ...
  const UpdateOrgStruItem = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/update/orgstructure/${parentId}/orgstructureitem/${itemId}/${profileId}/${Key_Key}`,
          {
            method: "PATCH",
            body: formData,
            credentials: "include",
        ...
        <Card.Body>
          {(innerdata || [])
            ...
            .map((filterItem, index) => (
              ...
              >
                <h4>Update Originational Structure</h4>
                {isError && (
                ...
                <Form.Group>
                  <Form.Label className="d-flex pt-1 justify-content-start">
                    <h5>
                      Image <span style={{ color: "gray" }}> Optional </span>{" "}
                    </h5>
                  </Form.Label>
                  <Form.Control
                    type="file"
...
const DeleteOriStruItem: React.FC<parentId> = ({ id }) => {
  ...
  const deleteItem = async (itemId: string) => {
    try {
      const response = await fetch(
        `http://165.22.219.69:5002/api/delete/orgstructure/${id}/orgstructureitem/${itemId}/${profileId}/${Key_Key}`,
        {
          method: "PATCH",
          credentials: "include",
        }
  ...
  useEffect(() => {
    // if(id){
    fetch(`http://165.22.219.69:5002/api/orgstructureitem/${id}`)
      .then((response) => response.json())
...
const AddOrgItem: React.FC<parentId> = ({ id }) => {
  const [validated, setValidated] = useState<boolean>(false);
  const [aboutText, setAboutText] = useState("");
  const [image, setImage] = useState<File | null>(null);
  const [showModal, setShowModal] = useState<boolean>(false);
  ...
  const addOrgItem = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/orgstructureitem/${id}/${profileId}/${Key_Key}`,
          {
            method: "POST",
            body: formData,
            credentials: "include",
      ...
      <Card>
        <Card.Body>
          <Form
            ...
            encType="multipart/form-data"
          >
            <h4>Add Originational Structure sequence wise</h4>
            {isError && (
              <Alert variant="danger" className="my-2">
                {isResponse}
              </Alert>
...
const OrgtnalStru = () => {
  ...
  const handleFormSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/orgstructure/${profileId}/${Key_Key}`,
          {
            method: "POST",
            body: formData,
            credentials: "include",
  ...
  const handleUpdate = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/update/orgstructure/${objectId}/${profileId}/${Key_Key}`,
          {
            method: "PATCH",
            body: formData,
            credentials: "include",
      ...
      <div>
        <Card>
          <Card.Body>
              ...
              {data.length === 0 ? (
                <Col lg={6}>
                  <Form
                    ...
                    encType="multipart/form-data"
                  >
                    <h4>Add Orignizational Structure</h4>
                    {isError && (
                      <Alert variant="danger" className="my-2">
                        {isResponse}
                      </Alert>
                ...
                <Col lg={6}>
                  <Form
                    ...
                    encType="multipart/form-data"
                  >
                    <h4>Update Orignizational Structure</h4>
                    {isError && (
                      <Alert variant="danger" className="my-2">
                        {isResponse}
                      </Alert>
            ...
            <Row>
              <Col lg={6}>
                <AddOrgItem id={objectId} />
              </Col>
              <Col lg={6}>
                {objectId && <DeleteOriStruItem id={objectId} />}
              </Col>
            </Row>
          </Card.Body>
        </Card>
