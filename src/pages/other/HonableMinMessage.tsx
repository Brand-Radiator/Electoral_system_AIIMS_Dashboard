...
const DeleteCmMessage = () => {
  ...
  const deleteItem = async (itemId: string) => {
    try {
      const response = await fetch(
        `http://165.22.219.69:5002/api/delete/honminmessage/${itemId}/${profileId}/${Key_Key}`,
        {
          method: "PATCH",
          credentials: "include",
        }
  ...
  useEffect(() => {
    fetch("http://165.22.219.69:5002/api/honminmessage")
...
const Introduction = () => {
  ...
  const handleFormSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/honminmessage/${profileId}/${Key_Key}`,
          {
            method: "POST",
            body: formData,
            credentials: "include",
  ...
  const handleUpdate = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/update/honminmessage/${objectId}/${profileId}/${Key_Key}`,
          {
            method: "PATCH",
            body: formData,
            credentials: "include",
            ...
            <Row>
              {introductionData.length === 0 ? (
                <Col lg={6}>
                  ...
                  >
                    <h4>Add Hon &apos;ble Minister Section</h4>
                    {isError && (
