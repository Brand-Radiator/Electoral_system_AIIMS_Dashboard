...
interface directorGeneralMessage {
  _id: string;
  bannerUrl: string;
...
const DeleteDirectorGenMessage = () => {
  ...
  const deleteItem = async (itemId: string) => {
    try {
      const response = await fetch(
        `http://165.22.219.69:5002/api/delete/dirgenmessage/${itemId}/${profileId}/${Key_Key}`,
        {
          method: "PATCH",
          credentials: "include",
        }
  ...
  useEffect(() => {
    fetch("http://165.22.219.69:5002/api/dirgenmessage")
  ...
  return (
    <>
      <>
        <h4>Delete Director General Info</h4>
...
const Introduction = () => {
  ...
  const handleFormSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/dirgenmessage/${profileId}/${Key_Key}`,
          {
            method: "POST",
            body: formData,
            credentials: "include",
  ...
  const handleUpdate = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/update/dirgenmessage/${objectId}/${profileId}/${Key_Key}`,
          {
            method: "PATCH",
            body: formData,
            credentials: "include",
            ...
            <Row>
              {introductionData.length === 0 ? (
                <Col lg={6}>
                  ...
                  >
                    <h4>Add Director General Info</h4>
                    {isError && (
