...
const NormalFormValidation = () => {
  ...
  const [title, setSportName] = useState("");
  const [sportName, setTitle] = useState("");
    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `${API_BASE_URL}/api/latestinformation/${profileId}/${Key_Key}`,
          {
            method: "POST",
            body: formData,
            credentials: "include",
        ...
        if (response.status == 201) {
          alert("Latest Information added successfully.");
...
const DeleteSponsors = () => {
  ...
  const deleteItem = async (itemId: string) => {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/delete/latestinformation/${itemId}/${profileId}/${Key_Key}`,
        {
          method: "PATCH",
          credentials: "include",
        }
      ...
      const data = await response.json();
      if (data) {
        alert("Latest Information deleted successfully.");
        window.location.reload();
      }
    } catch (error) {
  ...
  useEffect(() => {
    fetch("${API_BASE_URL}/api/latestinformation")
