    ...
    if (form.checkValidity() === false) {
      ...
      try {
        const response = await fetch(
          `http://165.22.219.69:5002/api/sponsor/${profileId}/${Key_Key}`,
          {
            method: "POST",
            body: formData,
            credentials: "include",
        ...
        <Card.Body>
          ...
          >
            ...
            <Form.Group>
              <Form.Label className="d-flex  pt-2justify-content-start font-weight-bold">
                <h5>
                  Sponsors Url <span style={{ color: "red" }}> *</span>
                </h5>
              </Form.Label>
              <Form.Control
                type="text"
...
const DeleteSponsors = () => {
  ...
  const deleteItem = async (itemId: string) => {
    try {
      const response = await fetch(
        `http://165.22.219.69:5002/api/delete/sponsor/${itemId}/${profileId}/${Key_Key}`,
        {
          method: "PATCH",
          credentials: "include",
        }
  ...
  useEffect(() => {
    fetch("http://165.22.219.69:5002/api/sponsor")
    ...
    <React.Fragment>
      <PageTitle
        breadCrumbItems={[
          ...
          { label: "Sponsors", path: "/forms/gallery", active: true },
        ]}
